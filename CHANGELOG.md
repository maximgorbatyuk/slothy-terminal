...
## [2026.3.18] - 2026-06-11

### Added
- **"Close Workspace" / "Close App" button in the empty-workspace welcome view.** `OpenFolderWelcomeView` is shown whenever the active workspace has no tabs (`hasEmptyActiveWorkspace`), but it previously offered no way *out* of an empty workspace — the only affordances were "open a folder" and the recent-folders list, so a workspace you'd emptied just sat there with no in-card exit. A bordered button now appears below the recent-folders section in that state: it reads "Close Workspace" and calls `AppState.closeWorkspace(id:)` (which then switches to another workspace), or reads "Close App" and quits via `NSApp.terminate(nil)` when this is the only workspace (`isLastWorkspace`, i.e. `workspaces.count <= 1`) so closing it wouldn't leave the app with nothing to show. Quitting deliberately routes through `NSApp.terminate` rather than a direct exit so the app's `willTerminateNotification` still fires for session cleanup and config save. The button is gated on `hasEmptyActiveWorkspace`, so it disappears the moment a tab exists. (`SlothyTerminal/Views/TerminalContainerView.swift:534-543, 580-595, 697-720`)

### Changed
- **Sidebar panel selector moved from a full-height edge strip to a labeled row at the midpoint of the sidebar.** The Files / Prompts / Scripts selector was a vertical, icon-only `SidebarTabStrip` pinned to the sidebar's leading or trailing edge and spanning the entire height — so the controls sat in the top corner while the panel they switch (the "changing block") lived in the bottom half, with the whole workspaces list wedged between them. The strip is now a horizontal `HStack` placed directly between the workspaces panel and the content panel, so the selector sits immediately above the block it controls. Each entry gained a text label beside its icon (a new `SidebarTab.title` returning "Files" / "Prompts" / "Scripts", kept distinct from the existing `tooltip` strings), and `SidebarTabIcon` was renamed to `SidebarTabButton` to reflect the icon-plus-label layout. The Git Client action moved to the trailing end of the row behind a `Spacer()` — it opens a tab, it does not switch panels, so the old separator rule between them is gone. The edge-flipping `HStack` keyed on `sidebarPosition` was removed from this view along with its now-unused `sidebarPosition` property; overall left/right placement of the sidebar still lives in `MainView`, so behaviour there is unchanged. (`SlothyTerminal/Views/SidebarContainerView.swift:43-94, 124-156`, `SlothyTerminal/Models/AppConfig.swift:251-264`)
- **Window title simplified to `📁 <directory> | SlothyTerminal`.** `windowTitleText` previously prefixed the active tab's directory with the workspace name whenever one resolved (`📁 <workspace> — <directory> | Slothy Terminal`), falling back to the directory alone otherwise. The workspace-name segment is dropped — the title is now always the directory plus the app name — and the app name itself loses its internal space, going from "Slothy Terminal" to "SlothyTerminal" to match the bundle name. The empty fallback (no active tab) changes for the same reason, from "Slothy Terminal" to "SlothyTerminal". (`SlothyTerminal/Views/MainView.swift:133-150`)

### Notes
- Verified with `swift test` (253 tests passing across 29 suites) and `xcodebuild -project SlothyTerminal.xcodeproj -scheme SlothyTerminal -configuration Debug build CODE_SIGNING_ALLOWED=NO` (BUILD SUCCEEDED). The app was not launched in this session — manual smoke recommended for: (a) the Files / Prompts / Scripts selector now sits as a labeled row in the middle of the sidebar, directly above the panel it switches, with the Git Client button at the right end, (b) move the sidebar to the left and to the right via settings — the selector row stays full-width and correctly placed in both, (c) empty a workspace of all its tabs — the welcome card shows a "Close Workspace" button that drops to another workspace, and "Close App" when it's the only one, (d) the window title shows just the directory followed by "SlothyTerminal".

## [2026.3.17] - 2026-06-04

### Added
- **Refresh-all button next to the status-bar usage bars.** A new `refreshButton` (an `arrow.clockwise` SF Symbol) sits to the left of the per-provider bars in `StatusBarUsageBars` and calls `usageService.fetchAll()` to re-pull every resolved provider at once; it doubles as a retry affordance for any provider stuck in `.failed`. Previously the only way to force a refresh was to wait for the background poll or to reopen the popover — there was no explicit "fetch now" control, and a transient network failure left a bar dead until the next scheduled poll. The button is disabled and dimmed to 0.4 opacity while any visible provider fetch is in flight (a new `isAnyLoading` computed property scans `usageService.status(for:)` for a `.loading` case), so rapid clicks can't stack overlapping fetches. To make room for it without regressions, the old single monolithic `HStack` was split into two subviews — `refreshButton` and `barsGroup` — so the refresh button sits *outside* the bars' tap target and hover highlight: clicking it neither opens the detail popover nor paints the group's hover background. (`SlothyTerminal/Views/StatusBarUsageView.swift:27-105`)
- **`DirectoryTreeExpansionStore` — persistence for sidebar folder expansion.** A new singleton service remembers which directory-tree folders are expanded, keyed by the tree's root directory path, and writes the state to its own JSON file at `~/Library/Application Support/SlothyTerminal/directory-tree-state.json`. Before this, the expanded/collapsed shape of the Files tree was pure `@State` on `DirectoryTreeView`/`FileItemRow`, so switching workspaces (which recreates those views) or relaunching the app collapsed everything back to the root — a real annoyance when working several folders deep. Writes are debounced at 0.5 s to absorb rapid folder toggling and flushed synchronously via `saveNow()` from the app's termination handler so the last toggle isn't lost on quit. The store is deliberately kept *out* of the observable `AppConfig`: routing high-frequency folder toggles through `ConfigManager.shared.config` would invalidate every view that reads config (the same reasoning as `ConfigManager.saveWindowFrame`). Collapsing a folder removes only that folder's own path, leaving remembered descendant paths intact so re-expanding restores the prior shape; load tolerates a missing or corrupt file by starting empty rather than crashing. (`SlothyTerminal/Services/DirectoryTreeExpansionStore.swift`, `SlothyTerminal/App/SlothyTerminalApp.swift:44-45`, `Package.swift:49`)
- **`DirectoryTreeExpansionStoreTests`** — 8 tests covering: unknown-root returns empty set, expanding records paths under the root, collapsing removes only the toggled folder's path, roots are isolated from each other, collapsing the last folder drops the root entry entirely, state survives across separate store instances (round-trips through disk), and a missing or corrupt backing file loads as empty state without crashing. (`SlothyTerminalTests/DirectoryTreeExpansionStoreTests.swift`)
- **`ANSIStripperTests`** — 6 tests covering: plain text and empty strings returned unchanged, CSI color and OSC sequences removed, cursor-movement sequences stripped while surrounding text is kept, and an all-escape-sequence input stripping to empty. (`SlothyTerminalTests/ANSIStripperTests.swift`)

### Changed
- **The Files block in the sidebar now grows to fill the available height instead of a fixed 300 pt cap.** `DirectoryTreeView`'s scrolling card was hard-capped at `.frame(maxHeight: 300)`, which on a tall window left the tree filling only ~60% of the explorer panel with dead space below it. The cap is now `.frame(maxHeight: .infinity)`, and the surrounding layout was adjusted to actually hand that space down: the trailing `Spacer()` in `SidebarView` (which had been absorbing all the slack and pinning the tree to the top) is removed in favour of `.frame(maxHeight: .infinity, alignment: .top)`, `TerminalSidebarView` gets the same fill-and-top-align so Project docs stays anchored at the bottom at its natural size, and `DirectoryTreeView` claims vertical space only while expanded (`maxHeight: isExpanded ? .infinity : nil`) so a collapsed Files section still shrinks to just its header row. (`SlothyTerminal/Views/SidebarView.swift:9-21, 51-60, 129-138`)
- **`DirectoryTreeManager.loadItems` / `loadChildren` now take an `expandingPaths` set and restore remembered expansion recursively.** Both gained an `expandingPaths: Set<String> = []` parameter; any scanned directory whose path is in the set is marked `isExpanded` with its children loaded eagerly (recursion is bounded by the finite set of remembered paths and bails early on `Task.isCancelled`). `DirectoryTreeView` seeds this from `DirectoryTreeExpansionStore.shared.expandedPaths(forRoot:)` on initial load, and `FileItemRow.toggleExpand()` writes each toggle back to the store and passes the remembered set when lazily loading a folder's children — so a folder reopened after a view recreation reopens its previously expanded subfolders too. The default empty-set argument keeps existing non-sidebar callers behaviourally unchanged. (`SlothyTerminal/Services/DirectoryTreeManager.swift:50-100`, `SlothyTerminal/Views/SidebarView.swift:155-156, 323-352`)
- **`ANSIStripper.strip` short-circuits text with no escape byte before touching the regex engine.** Every alternative in the strip pattern begins with `ESC` (`0x1B`), so a string containing no `ESC` can never match. A `guard text.contains("\u{1B}")` fast path now returns such input unchanged before the regex runs. This matters because the function is on the hot terminal-output path — the grid text read back from libghostty per snapshot is usually already plain — and running a full regex pass over every clean snapshot was avoidable work. (`SlothyTerminal/Services/ANSIStripper.swift:6-14`)

### Notes
- Documentation updated alongside the code: `docs/architecture.md` and `docs/interactions.md` now record the new `directory-tree-state.json` persistence file (location, owner, write semantics), and `docs/testing.md` lists `DirectoryTreeExpansionStore` under SPM-covered service tests. (`docs/architecture.md`, `docs/interactions.md`, `docs/testing.md`)
- Verified with `swift test` (253 tests passing across 29 suites, including the 8 new `DirectoryTreeExpansionStore` tests and 6 new `ANSIStripper` tests) and `xcodebuild -project SlothyTerminal.xcodeproj -scheme SlothyTerminal -configuration Debug build CODE_SIGNING_ALLOWED=NO` (BUILD SUCCEEDED). The app was not launched in this session — manual smoke recommended for: (a) expand several folders deep in the Files sidebar, switch to another workspace and back — the tree keeps its expanded shape, (b) quit and relaunch — expansion still restored, (c) resize the window taller — the Files tree grows to fill the panel with Project docs pinned at the bottom, (d) click the new refresh icon next to the usage bars — all providers re-fetch and the icon dims until they settle.
...
### Added
- **Native text/code editor as a first-class tab mode (`.editor`).** Double-clicking a file in the sidebar's directory tree now opens it in an editor tab inside the current workspace instead of copying the relative path. The tab carries a `fileURL`, an `isDirty` flag, and renders an `STTextView`-backed buffer with syntax highlighting for Swift and Markdown via tree-sitter, line numbers, and current-line highlighting. The tab's `title` and `tabName` are computed from `fileURL.lastPathComponent` so a future Save As that mutates `fileURL` stays in sync without a separate write; dirty tabs are prefixed with a `●` marker. The editor uses a file-type-driven theme — `.md` / `.markdown` / `.txt` open on a paper-white canvas with dark text, everything else opens on a dark canvas with colored highlights — chosen deliberately *not* to follow system appearance so a Swift file always looks like code and a Markdown draft always looks like prose. (`SlothyTerminal/Models/Tab.swift:6, 21-22, 37, 41-72, 88-92, 100-128, 145-149, 374-376`, `SlothyTerminal/Views/Editor/EditorTabView.swift`, `SlothyTerminal/Views/Editor/EditorTheme.swift`, `SlothyTerminal/Views/Editor/EditorLanguage.swift`, `SlothyTerminal/Views/Editor/SyntaxHighlightingPlugin.swift`, `SlothyTerminal/Views/SidebarView.swift:172-242`, `SlothyTerminal/Views/TabBarView.swift:248-250`, `SlothyTerminal/Views/TerminalContainerView.swift:194-198, 308-313`)
- **`FileEditorService` — disk I/O for the editor.** Load/save through a single `FileHandle` so size, binary sniff, and content read come from the same descriptor (closes the TOCTOU window between separate `attributesOfItem` / `Data(contentsOf:)` calls). Enforces a 10 MB inline cap on both load and save; refuses files with NUL bytes in the first 8 KB; resolves symlinks before writing so editing a symlinked dotfile preserves the link instead of replacing it with a regular file. Encoding-fallback chain on load is `utf8 → windowsCP1252 → macOSRoman → isoLatin1` — the order matters because `isoLatin1` is total and would shadow CP1252's smart-quote bytes (`0x81 / 0x8D / 0x8F / 0x90 / 0x9D` undefined) if put first; UTF-16 is intentionally absent because almost any even-length byte stream decodes under it. Maps Cocoa `NSCocoaErrorDomain` permission codes to a typed `.permissionDenied` so callers don't grep error strings. (`SlothyTerminal/Services/FileEditorService.swift`, `Package.swift:51`)
- **`AppState.openFileInEditor(_:)`** — single entry point for opening files. Globally dedupes editor tabs across workspaces (canonicalizes the URL through `resolvingSymlinksInPath().standardizedFileURL` so `/tmp/...` and `/private/tmp/...` collapse to one identity, and switches workspace if needed to focus the existing tab). When no existing tab matches, picks the directory passed to `resolveWorkspaceID` from `activeWorkspace.rootDirectory → currentContextDirectory → fileURL.parent` in that order — falling straight to the file's parent would let a nested file (e.g. `~/proj/Models/Tab.swift`) silently retarget the active workspace away from `~/proj`. The new tab's `workingDirectory` is the *workspace root*, not the file's parent, so callers like `gitBranchRefreshContext` and the `+` button keep pointing at the workspace the user picked. (`SlothyTerminal/App/AppState.swift:411-466`)
- **Dirty-editor close confirmation flow.** Closing a dirty editor tab — whether via Cmd+W, the tab's close button, or any other path through `AppState.closeTab(id:)` — routes through a new `tabPendingDirtyEditorClose` state and the editor's `.confirmationDialog` instead of dropping unsaved changes. If the dirty tab isn't the active tab (or isn't in the active workspace), `closeTab` first snapshots the current `(activeWorkspaceID, activeTabID)` into `dirtyEditorCloseReturnContext`, switches to the dirty tab so the user can see what's being closed, then surfaces the sheet; "Cancel" restores the snapshot, "Don't Save" clears `isDirty` and proceeds, "Save" runs the save first and closes on success. `performCloseTab` carries a `precondition(!(mode == .editor && isDirty), …)` so any future code path that bypasses the sheet fails loudly in Debug. (`SlothyTerminal/App/AppState.swift:137-143, 493-595`, `SlothyTerminal/Views/Editor/EditorTabView.swift:124-145, 235-275`)
- **File menu — Save / Save As… / Revert to Saved.** `EditorFileMenuItems` reads `editorSave` / `editorSaveAs` / `editorRevert` from `FocusedValues` and renders the three commands as a divider-separated block under the existing close-tab entry. Items render unconditionally even when no editor is focused (only `.disabled` flips), because letting Cmd+S fall through to a focused terminal would send `^S` (XOFF / pause output) and visibly freeze the foreground process — the chord must stay claimed app-wide. Save As resolves a destination through `NSSavePanel` with the current `fileURL` as the suggested name and re-attaches the tab to the new URL on success; Revert routes through an `NSAlert` confirmation and re-reads from disk. (`SlothyTerminal/App/SlothyTerminalApp.swift:99-104`, `SlothyTerminal/Views/Editor/EditorFileMenuItems.swift`, `SlothyTerminal/Views/Editor/EditorTabView.swift:113-115, 553-617`)
- **Editor font picker (Settings → Appearance → Editor Font).** New `AppConfig.editorFontName` (default `"SF Mono"`) and `AppConfig.editorFontSize` (default `13pt`, slider range `10...24`), independent of `terminalFontName` / `terminalFontSize` so a user can keep a chunky ligature-friendly font for editing while leaving the terminal on the platform default. Live Swift-flavored code preview inside the settings section renders in the picked family + size. The picker also drives `EditorTabView`'s STTextView by writing the font into the bound `AttributedString`'s `.font` attribute (not via SwiftUI's `\.font` environment — the STTextView wrapper resets `attributedText` from the binding on every `updateNSView`, which would otherwise wipe a per-view font modifier on tab switches); a single `.onChange(of: "\(name)#\(size)")` rebakes the font across the buffer when settings change. Decoder is resilient — missing keys fall back to defaults instead of failing the whole `AppConfig` parse. (`SlothyTerminal/Models/AppConfig.swift:54-60, 154-155`, `SlothyTerminal/Views/SettingsView.swift:274-310`, `SlothyTerminal/Views/Editor/EditorTabView.swift`)
- **Three new SwiftPM/Xcode dependencies for the editor stack.** `STTextView` 2.3.10 (text view + line numbers + plugin hooks), `Neon` 0.6.0 (highlighter glue — pinned with `SwiftTreeSitter` 0.8.0 *exact* because 0.9.0+ broke Neon 0.6.0 with `@MainActor`), `tree-sitter-swift` pinned to commit `7b7909f2f6b9414be0958275f4c8e5d69c3bca43`, and `tree-sitter-markdown` on the `split_parser` branch. Resource-bundle resolution in `EditorLanguage.bundleContainingHighlights(for:)` uses an *exact* suffix match on `<package>_<hint>.bundle` so `TreeSitterMarkdown` doesn't accidentally load the sibling `TreeSitterMarkdownInline` bundle's `queries/highlights.scm`. Bundles ship in the app target only (`SlothyTerminal.xcodeproj/project.pbxproj`); `Package.swift` deliberately doesn't reference them so SPM tests stay free of GhosttyKit-style native blockers. (`SlothyTerminal.xcodeproj/project.pbxproj:7-13, 51-58, 100-109, 137-143, 393-440`, `SlothyTerminal/Views/Editor/EditorLanguage.swift:75-126`)
- **`EditorTabTests`** — 16 tests covering: file-name vs parent-directory titling, the `●` dirty marker on `tabName`, `title`/`tabName` following `fileURL` mutations (Save As), `isExecuting` always false for editor tabs, `openFileInEditor` create-and-switch behavior, focus-existing dedup, workspace-root vs file-parent `workingDirectory`, the "nested file doesn't retarget empty workspace" guard, symlinked-path canonicalization, the dirty-close routing flow (active tab, inactive tab, cancel restores prior context, discard finishes the close), cross-workspace focus, and the `excludingTabID` parameter on `hasOpenEditorTab`. All test URLs derive from `FileManager.default.temporaryDirectory` (already symlink-resolved on macOS to `/private/var/folders/...`) so they survive `canonicalFileURL` without surprise mismatches. (`SlothyTerminalTests/EditorTabTests.swift`)
- **`FileEditorServiceTests`** — 13 tests covering: UTF-8 round-trip, NUL-byte binary refusal, size-cap refusal on both load and save, missing-path handling, empty-file load, CP1252-before-isoLatin1 ordering (smart quotes survive), `unrepresentableOnSave` for chars outside the chosen encoding, NUL-in-buffer refusal on save, symlink preservation on save (write through, not replace), and the empty-file and missing-path branches of `isProbablyBinary`. (`SlothyTerminalTests/FileEditorServiceTests.swift`)

### Changed
- **Sidebar double-click on a file now opens it in the editor instead of copying the relative path.** The old `count: 2` tap gesture (copy-to-clipboard) is removed from `FileItemRow` and replaced with `appState.openFileInEditor(item.url)`. Crucially the double-click handler is attached *before* the `count: 1` handler — SwiftUI's gesture recognizer resolves higher-count gestures first, and a single-then-double ordering would let the single-tap handler win and swallow the second click. The "Copy Relative Path" action survives as a context-menu item for users who want it. (`SlothyTerminal/Views/SidebarView.swift:172-242`)
- **`Tab` initializer is stricter and now uses `precondition`.** `agentType` may now only be set on `.terminal` tabs (previously the assertion only blocked `.git` + agentType); `.editor` tabs must carry a `fileURL`. The asserts became `precondition`s so a mis-call fails the same way in Release as in Debug — the editor flow assumes these invariants hold at every read site and we'd rather crash than silently render a broken tab. (`SlothyTerminal/Models/Tab.swift:88-92, 100-117`)
- **`Tab.title` is now a computed property for editor tabs.** Setter is a no-op + `assertionFailure` for `.editor` mode (the getter always returns `fileURL?.lastPathComponent`, so a stored write is unobservable and almost certainly a bug). Non-editor tabs are unchanged — `storedTitle` carries the original behavior. (`SlothyTerminal/Models/Tab.swift:37, 41-72`)
- **`ActiveTerminalView` short-circuits PTY/chat setup for editor tabs.** The `.task` block that boots agent processes and PTY surfaces now returns early on `tab.mode == .git || tab.mode == .editor` (previously only `.git`), so opening a file doesn't spawn a shell process. (`SlothyTerminal/Views/TerminalContainerView.swift:308-313`)

### Notes
- **Selection binding is intentionally `.constant(nil)`** in `EditorTabView`'s `TextView(...)`. STTextViewSwiftUI publishes selection changes through the binding on every click and drag, which forces SwiftUI to call `updateNSView`, which re-assigns `attributedText`, which calls STTextView's `setString` and wipes the layout manager's rendering attributes — making the syntax colors flash off and back on. We don't read selection anywhere; if a future feature needs the caret position, observe it through STTextView's delegate rather than re-introducing the binding.
- **Why baked-in font and not the SwiftUI env.** `STTextViewSwiftUIAppKit` shadows SwiftUI's `\.font` environment key with its *own* `NSFont`-typed key (`FontEnvironmentKey` in `TextViewModifier.swift`), and only re-applies the font when the wrapper's stored `lastFont` differs from the incoming one — false after the first apply. Embedding `.font` into the `AttributedString` storage attributes is what survives the wrapper's `attributedText =` reset cycle. Two onChanges (`editorFontName`, `editorFontSize`) were collapsed into one watching a `"name#size"` string because two adjacent onChanges plus the surrounding `switch` chain tipped SwiftUI's expression-complexity checker over.
- **Bundle resolution is suffix-strict.** `EditorLanguage.bundleContainingHighlights(for:)` matches `TreeSitterMarkdown` against `<package>_TreeSitterMarkdown.bundle` but rejects `<package>_TreeSitterMarkdownInline.bundle`. Both bundles ship `queries/highlights.scm` but expose different grammars (block vs inline), and a substring-match would silently load the inline queries for the block parser, producing zero captures.
- **Editor files are app-only.** `EditorTabView.swift`, `EditorTheme.swift`, `EditorLanguage.swift`, `SyntaxHighlightingPlugin.swift`, and `EditorFileMenuItems.swift` import STTextView / SwiftTreeSitter / SwiftUI and are deliberately *not* listed in `Package.swift` `sources:`. `FileEditorService.swift` is pure Foundation and *is* in the SPM sources list, which is what lets the 13 `FileEditorServiceTests` run under `swift test`. The 16 `EditorTabTests` only touch `Tab` and `AppState`, so they also run under SPM despite the editor view being Xcode-only.
- Verified end-to-end with `swift test` (239 tests passing, including the 29 new editor tests) and `xcodebuild -project SlothyTerminal.xcodeproj -scheme SlothyTerminal -configuration Debug build CODE_SIGNING_ALLOWED=NO` (BUILD SUCCEEDED). Manual smoke recommended for: (a) double-click a `.swift` file → editor opens with colored highlights, (b) double-click an `.md` file → editor opens on light canvas with dark text, (c) edit a buffer → tab name gets `●` prefix → Cmd+W → confirmation sheet appears → Cancel returns to where you were, (d) change the editor font in Settings → both open editor tabs update live without losing the per-tab font on subsequent tab switches.
