...
/// Task runner that wraps `OpenCodeCLITransport` for headless execution.
///
/// Key difference from Claude: OpenCode spawns one process per message,
/// so `send()` is called immediately after `start()` (no waiting for `onReady`).
class OpenCodeTaskRunner: TaskRunner {
  private var transport: OpenCodeCLITransport?
  private var isCancelled = false
  private var forceKillTask: Task<Void, Never>?

  private let workingDirectory: URL
  private let selectedModel: ChatModelSelection?
  private let selectedMode: ChatMode?
  ...
  func execute(task: QueuedTask, logCollector: TaskLogCollector) async throws -> TaskRunResult {
    ...
    let transport = OpenCodeCLITransport(
    ...
    logCollector.append("Starting OpenCode CLI transport")
    logCollector.append("Working directory: \(workingDirectory.path)")
    if let selectedModel {
      logCollector.append("Model: \(selectedModel.cliModelString)")
    }
    if let selectedMode {
      logCollector.append("Mode: \(selectedMode.rawValue)")
    }
    ...
    return try await withCheckedThrowingContinuation { continuation in
      ...
      /// OpenCode doesn't wait for onReady — send immediately.
      logCollector.append("Sending prompt immediately (\(task.prompt.count) chars)")
  ...
  func cancel() {
    ...
    isCancelled = true
    Logger.taskQueue.info("OpenCodeTaskRunner: cancelling")
    ...
    forceKillTask = Task { [weak self] in
      ...
      Logger.taskQueue.warning("OpenCodeTaskRunner: force-killing after 10s")
      transport.terminate()
    }
  }
